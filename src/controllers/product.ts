...
export const newProduct = TryCatch(
    async (req: Request<{}, {}, NewProductRequestBody>, res, next) => {
        ...
        if (!name || !price || !stock || !category) {
            // Remove the uploaded photo if there's an error
            rm(photo.path, (err) => {
                if (err) {
                    console.error("Error deleting photo:", err);
                } else {
                    console.log("Deleted uploaded photo due to missing fields.");
                }
            });
        ...
        const normalizedPhotoPath = photo.path.replace(/\\/g, '/');
...
export const deleteProduct = TryCatch(async (req, res, next) => {
    ...
    // // Check if the product has a photo and remove it if so
    // if (product.photo) {
    //     rm(product.photo, (err) => {
    //         if (err) {
    //             console.error("Error deleting photo:", err);
    //             return next(new ErrorHandler("Error deleting product photo", 500));
    //         }
...
// const generatedRandomProducts = async (count: number = 10) => {
//     const products = [];

//     for (let i = 0; i < count; i++) {
//         const product = {
//             name: faker.commerce.productName(),
//             photo:"uploads\\deec31ee-3b95-48e6-bffc-20fa65f7c400.jpg",
